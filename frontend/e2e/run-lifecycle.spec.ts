...
test("runs, pauses, seeks, and ends a city simulation", async ({ page }, testInfo) => {
  ...
  if (testInfo.project.name === "desktop") {
    const boxes = await page.locator(".menu-drawer, .city-map, .timeline").evaluateAll((elements) =>
    ...
    expect(boxes[0].left).toBe(320);
    expect(boxes[0].right).toBe(1440);
    expect(boxes[1].left).toBe(0);
    expect(boxes[1].right).toBeLessThanOrEqual(boxes[0].left + 1);
    expect(boxes[0].bottom).toBeLessThanOrEqual(boxes[2].top + 1);
  }
...
test("generates random towns with people, vehicles, and heat", async ({ page }, testInfo) => {
  ...
  const firstTown = await page.locator(".map-title h1").innerText();
  await page.getByRole("button", { name: "生成城镇" }).click();
  await expect.poll(() => page.locator(".map-title h1").innerText(), { timeout: 20_000 }).not.toBe(firstTown);
  await expect(page.locator(".scenario-meta").filter({ hasText: "已就绪" })).toBeVisible();
