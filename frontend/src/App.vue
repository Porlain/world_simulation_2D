...
const flowId = computed(() => selectedBundle.value?.config.flow_types[0]?.id ?? null);
...
async function startRun(seed?: number) {
  ...
  error.value = null;
  try {
    const detail = await createRun(selectedScenarioId.value, seed);
...
async function pollLatest() {
  ...
  try {
    const detail = await getRun(runId, false, snapshotAbort.signal);
...
function seek(tick: number) {
  ...
  seekTimer = window.setTimeout(async () => {
    ...
    snapshotAbort = new AbortController();
    try {
      const snapshot = await getSnapshot(runId, tick, snapshotAbort.signal);
  ...
  <div class="app-shell">
    <header class="topbar">
      ...
      <div class="topbar-readout" aria-live="polite">
        ...
        <span>{{ statusLabel(selectedRun?.status ?? null) }}</span>
        ...
        <aside class="stats-panel" aria-label="流量统计">
          ...
          <section class="stats-section">
            ...
            <dl v-else-if="selectedConnection && displayedSnapshot && flowId" class="metric-list">
              <div><dt>刚刚出发</dt><dd>{{ formatNumber(displayedSnapshot.connection_activity[selectedConnection.id]?.[flowId]?.departed ?? 0) }}</dd></div>
